...
### Deeper Interpretation — Why the Numbers Behaved This Way

The most valuable insight from this study is that **the average latency is misleading on its own**. Across all three tests the average stayed near 280–310 ms, which would suggest "healthy" performance. But the gap between the median (~140–250 ms) and the maximum (up to 8.97 s) reveals a heavily **right-skewed latency distribution** — most requests are fast, but a long "tail" of slow requests hides behind a comfortable-looking average. This is exactly why professional performance engineers report **percentiles (p90/p95)** and the maximum rather than the average alone: the tail is where real users experience pain.

Three concrete mechanisms explain the observed behaviour:

1. **Connection warm-up (cold start).** The load test executed first. Before any requests, the client must perform DNS resolution, a TCP handshake, and a TLS negotiation for new connections. These one-time costs were amortised over a relatively small request count (1,404), so they inflated the load test's p95 (1050 ms) and produced the 8.40% of checks that breached the 500 ms threshold. By the time the stress and spike tests ran, connections were already warm and reused, so their p95 values were actually *lower* despite far more load.

2. **Connection pooling and HTTP keep-alive.** k6 reuses TCP connections between iterations. As virtual users rose to 300, the pool of warm connections grew, allowing the higher-load tests to sustain 100+ req/s with stable median latency. This is why throughput scaled almost linearly (11.55 → 101.70 → 112.12 req/s) without a collapse — the system never reached its saturation point.

3. **Tail latency from shared infrastructure.** JSONPlaceholder is a free, shared, CDN-fronted mock service. The occasional multi-second maximum response times are consistent with transient queueing, rate-limiting, or a cache miss on the shared backend — not with the server failing, since `http_req_failed` remained 0.00% throughout.

### Discussion — Did the Hypothesis Hold?

My hypothesis was **partly wrong, in an instructive way** — and being wrong here taught me more than being right would have:

- I predicted normal load would be the *fastest and cleanest* test. In reality it had the *worst* p95 (1050 ms) and the *highest* check-failure rate (8.40%), because of the cold-start effect described above.
- I predicted errors would appear and grow as VUs climbed past 100–200. Instead, the API handled 300 concurrent users with **zero HTTP failures** and delivered *better* p95 latency than the warm-up load test, demonstrating that JSONPlaceholder's true capacity ceiling lies well beyond 300 VUs.
- My spike prediction was the most accurate: the sudden burst was survived cleanly (100% successful checks), with the only cost being a few very slow requests (5.53 s max) during the surge.

This contrast between prediction and reality is the core learning of the project: **intuition about performance is unreliable, and only empirical measurement reveals where a system's real weaknesses lie.**
