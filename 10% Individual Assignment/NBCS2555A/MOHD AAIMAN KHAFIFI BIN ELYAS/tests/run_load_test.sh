...
mapfile -t LONGEST_VALUES < <(grep "Longest transaction:" ../error_logs/load_errors.txt | awk '{print $3}')

# 2. Add a clean header to your new output file
echo -e "\tDate & Time,  Trans,\tElap Time,\tData Trans,\tResp Time,\tTrans Rate, \tThroughput,\tConcurrent,\tOKTx,\tFailTx, Longest_Transaction," > ../results/load_test_final.csv
...
do
    ...
    if [[ ! "$line" =~ ^# ]] && [[ -n "$line" ]]; then
        ...
        echo "${line},${MAX_LATENCY}" >> ../results/load_test_final.csv
        
        # Advance row index counter
        ((LINE_COUNT++))
    fi
done < "../results/load_test.csv"

rm -f ../results/load_test.csv